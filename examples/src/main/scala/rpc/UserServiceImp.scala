...
@Singleton
class UserServiceImpl @Inject()(userStore: UserStore)
    extends UserServiceGrpc.UserService {

  override def findUsers(request: UserService.FindUsersRequest) =
    Future.successful {
      val users = userStore
        .all()
        .filter {
          case (id, user) =>
            (request.userIds.isEmpty || request.userIds.contains(UserId(id))) &&
              (request.emails.isEmpty || request.emails.contains(user.email))
        }
        .toList
        .map { case (id, user) => UserConversions.courierToProto(id, user) }
      UserService.KeyedUsers(users)
    }

  override def createUser(request: UserService.User) = Future.successful {
    UserId(userStore.create(UserConversions.protoToCourier(request)))
  }
